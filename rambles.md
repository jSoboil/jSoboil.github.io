...
<img src="img/cdf_1.png" width="620" height="510" align="left" />
...
We can also apply the method to generate a random sample from the exponential distribution $$f(x) = \lambda\exp{-\lambda x}$$, with mean $$\frac{1}{\lambda}$$. If $$X \sim exp{\lambda}$$ then for $$x > 0$$ the cdf  is $$F_{X}(x) = 1 - \exp^{-\lambda * x}$$. 
